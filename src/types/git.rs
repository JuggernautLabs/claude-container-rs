//! Git state types — the (Container, Host) pair and two-leg state model.
//!
//! Both sides use the same enum. RepoState is derived from the pair.
...
/// RepoState is derived from matching on this pair via repo_state().
///
