...
// ============================================================================
// 6. Content-identical squash doesn't trigger false pending merge
// ============================================================================

#[test]
fn squash_identical_content_skips_merge() {
    use git_sandbox::types::git::*;
    use git_sandbox::types::CommitHash;

    // Simulate: session branch and main have different SHAs but identical trees
    // (this happens after squash-merge)
    let relation = SessionTargetRelation {
        ancestry: Ancestry::Diverged {
            merge_base: Some(CommitHash::new("aaa")),
            container_ahead: 1,
            host_ahead: 1,
        },
    ...
    let pair = RepoPair {
        name: "test".into(),
        container: GitSide::Clean { head: CommitHash::new("abc") },
        host: GitSide::Clean { head: CommitHash::new("abc") },
        ...
        target_head: Some(CommitHash::new("def")),
        session_to_target: Some(relation),
    };

    let decision = pair.sync_decision();
    // Should NOT be MergeToTarget because content is identical
    assert!(!matches!(decision, SyncDecision::MergeToTarget { .. }),
        "Identical content after squash should NOT trigger MergeToTarget. Got: {:?}", decision);
}
